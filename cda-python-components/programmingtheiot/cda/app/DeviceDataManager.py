...
class DeviceDataManager(IDataMessageListener):
	...
	def __init__(self):
		...
		self.enableActuation = self.configUtil.getBoolean(section=ConfigConst.CONSTRAINED_DEVICE, key = ConfigConst.ENABLE_ACTUATION_KEY)
	...
	def handleSensorMessage(self, data: SensorData) -> bool:
		...
		"""
		if data: 
			logging.debug(f"Incoming sensor data recieved (from sesnor manager): {str(data)}")
	...
	def handleSystemPerformanceMessage(self, data: SystemPerformanceData) -> bool:
		...
		"""
		if data: 
			logging.debug(f"Incoming system performance message recieved (from system performance manager): {str(data)}")
			return True
		else: 
			logging.warning(f"Incoming system performance data is invalid (null). Ignoring.")
			return False
	...
	def stopManager(self):
		logging.info("Stoping DeviceDataManager....")
	...
	def _handleIncomingDataAnalysis(self, msg: str):
		...
		if isinstance(msg,ActuatorData):
			try: 
				msg = ActuatorData(d=msg)
			except: 
				logging.warning (f"Could not convert {msg} to actuator data type")
			
			logging.debug(f"sending {msg} to actuator")
			self.actuatorAdapterMgr.sendActuatorCommand(msg)
